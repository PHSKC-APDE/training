...
#dplyr
dat_dplyr %>% summarize(est = mean(api00), se = sd(api00)/sqrt(n())) %>% mutate(lci = est - 1.96 * se, uci = est + 1.96 *se)

#data.table
dat_dt[, .(est = mean(api00), se = sd(api00)/sqrt(.N))][, .(est, se, lci = (est - 1.96 * se), uci = (est + 1.96 * se))]

#srvyr
dat_svy %>% summarize(est = survey_mean(api00), ci = survey_mean(api00, vartype = 'ci')[2:3]) %>% rename(se = est_se ,lci = ci_low, uci = ci_upp)
...
#dplyr
dat_dplyr %>% group_by(stype) %>% summarize(est = mean(api00), se = sd(api00)/sqrt(n()))

#data.table
dat_dt[, .(est = mean(api00), se = sd(api00)/sqrt(.N)), by = 'stype']
...
m_new <- data.frame(letter = c('A','B'), num = 1:2)
...
merge(m_base, m_new, by = 'latter', all =T)
...
dance = function(x) paste0(rep('D', x), 'ance')
dance(1)
dance(5)
