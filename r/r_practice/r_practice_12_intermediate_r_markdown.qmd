...
The same techniques shown in the beginner markdown guide, alongside some careful use of code chunks, will suffice for most use cases. For more complex projects, one may wish to structure the content in the form of an analysis plan. This is the approach outlined here.

## Analysis plan as a living project document

Analysis plans are documents that outline the work to be undertaken by the analyst. The are, ideally, drafted early in the conception of a project, as a way to make sure all stakeholders are "on the same page". But they also should be kept up to date throughout the project. An up-to-date analysis plan serves three purposes: 

1: It serves as a single location for direct contributors (e.g. the analyst) to document critical information about the purpose, methods, and expected outputs of their work.
2: It serves as a store of langauge and key informaiton crucial to facilitate management of the work by senior/management and by other consumers of the analyst work
3: It serves as a briefing of the project that may be referenced by out future selves to quickly orient us to what was done and why

These are potentially valuable goals, but achieving them requires keeping the analysis plan up-to-date throughout the project. Here we show how markdown can greatly reduce the burden of this, by marrying documentation and work into a single document.

## Analysis plan as structure in quantitative project work
...
Because an analysis plan starts as a planning document and becomes internal documentation of a project, the sections serve different purposes than would be expected in publications:

1: Bakground/Introduction
...
Documents the analyst' intentions and design decisions. This should be in plain, but precise langauge (most often the analyst is writing to themself) and sturctured to be easy to browse. Particularly helpful are where th
