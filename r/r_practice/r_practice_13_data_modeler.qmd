...
[this document](https://github.com/PHSKC-APDE/training/blob/main/r/r_practice/r_practice_13_data_modeler.qmd)

rads::data_moderle creates a simulated dataset structure where the underlying data are uncorrelated across columns. Once uncorrelated the data is no longer informative for any individual row. However, the data within a particular column will be representative of the underlying distributions when the data has been classified as categorical by data_modeler. For non-categorical data, text fields the text field will be roughly the same length as what is in the underlying data, although the actual text would not be reflective of the data. Non-categorical numbers will be from a uniform distribution of numbers with the same min-max and precision as the source data.                                                                    
...
DT.synthetic.data <- rads::data_modeler(ph.data = DT.real.data, #our "real" data
                              ...
                              print_code = T) #print the generation code to terminal. useful for both review and if you wish to copy into other contexts, such as a shareable script or in a chat with a LLM/AI
...
*not all data types use these, and they are still a work-in-progress*

# In Summary:

- Proportion of response for each indicator categorical is spread across all data. No relationship exists between indicator and outcome at observation level.  For categorical variables, column would be representative of underlying distribution 

- For noncategorical columns modeler would use min/max values; would not have ability to show peaks of distribution. 

- Variable type detection depends on having sufficient observations in the original data (particularly for non-categoricals). For most use cases, 1000 or more observations should work. (if something shouldn’t be categorical, but isn’t variable enough, adding more observations or reducing the detection threshold can have it properly be detected as non categorical.) 

   - More exactly: having 2 orders of magnitude more observations than levels in the largest category should work (so if the largest category is 9 or fewer levels, having 100 observations in the input data should work, but if 10-99 levels (such as our HRAs), having 1000, and if 100-999 (e.g. a large number of zip codes) 10000 observations would be better.  

- Pare down variables list for data modeler, not recommended to use all available variables in a given data set 

- Set “seed” before running code to generate same output for two users running data modeler 

- When running code, select “print code”= “true” to review parameters that modeler is defining/using for each variable in original data set to create synthetic data set.  User could manually add additional anonymization as needed  
